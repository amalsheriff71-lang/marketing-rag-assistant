...
# --- Page Configuration ---
st.set_page_config(
    page_title="Marketing Intelligence Assistant",
    page_icon="📊",
    layout="wide"
)
...
if st.button("Generate Insights"):
    # Priority: st.text_input > os.environ (Streamlit Secrets)
    current_api_key = api_key if api_key else os.environ.get("OPENAI_API_KEY")
    
    if not current_api_key:
        st.warning("⚠️ Please enter your OpenAI API Key in the sidebar or set it in Streamlit Secrets.")
    elif query:
        with st.spinner("🧠 Analyzing social media data..."):
            ...
            llm = ChatOpenAI(model_name="gpt-3.5-turbo", temperature=0, openai_api_key=current_api_key)
